    ...
    public async Task<HttpResponseMessage> SendAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    HttpRequestMessage req = new HttpRequestMessage(request.Method, request.RequestUri);
                    req.Content = request.Content;
                    req.AppendHeaders(request.Headers);
					// 发送请求
                ...
                }, RetryPolicy.Value.NumRetries, RetryPolicy.Value.RetryTimeout, fallbackPolicy: async exception =>
                {
                    // 单次失败时，需要重试时，执行
                    // ... todo
        ...
        // 请求成功
        if ((response?.IsSuccessStatusCode == true || response?.StatusCode == HttpStatusCode.Unauthorized || response?.StatusCode == HttpStatusCode.InternalServerError || response?.StatusCode == HttpStatusCode.ServiceUnavailable) && exception == default)
        {
            // 打印成功请求
